<x-guest-layout>
    <div class="w-full sm:max-w-md px-6 py-8 bg-white/90 backdrop-blur-sm shadow-xl overflow-hidden sm:rounded-lg border border-gray-100">
        ...
        <h2 class="text-center text-lg font-medium mb-6 text-blue-600">Zaloguj się do swojego konta</h2>
        ...
        <form method="POST" action="{{ route('login') }}">
            ...
            <div class="flex flex-col space-y-4 mt-6">
                ...
                @if (Route::has('password.request'))
                    <a class="text-sm text-center text-gray-600 hover:text-blue-600" href="{{ route('password.request') }}">
                        Zapomniałeś hasła?
                    </a>
                @endif
            </div>
            ...
            <div class="flex items-center justify-center mt-6 pt-4 border-t border-gray-200">
                ...
                <a class="ml-1 text-sm font-medium text-blue-600 hover:text-blue-700" href="{{ route('register') }}">
                    Zarejestruj się
                </a>
            </div>
        </form>
